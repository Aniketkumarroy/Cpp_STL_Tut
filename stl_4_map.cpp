...
int main() {
  ...
  /*
      std::map<key_type, value_type>, key should be unique in map, each key will
     correspond to a value. in map the key value pair is stored in a sorted
     order, ofcourse you can provide a custom sort function
  ...
  */
  ...
  auto it3 = std::find_if(
      m1.begin(), m1.end(),
      [](const std::pair<int, std::string> &pair) { return pair.first == 0; });
  if (it3 != m1.end()) {
