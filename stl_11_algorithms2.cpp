...
int main() {
  ...
  if (result.first != s1.end() && result.second != s2.end()) {
    std::cout << "The first mismatch is at index "
              << std::distance(s1.begin(), result.first) << ":\n";
    std::cout << "s1: " << *result.first << ", s2: " << *result.second
              << std::endl;
  }
  s1[13] = s2[13];
  ...
  if (it1 != v.end())
    std::cout << "found 3 consecutive 1 at index "
              << std::distance(v.begin(), it1) << "\n";
  // ******************************* move *******************************
  std::vector<std::string> source = {
      "it",   "transfer",   "ownership", "instead", "of",
      "copy", "preferable", "for",       "memory",  "optimization"};
